...
// a 复制 pcb 所在的整个页, 里面包含进程 pcb 信息及特权 0 级的栈, 里面包含了返回地址, 然后再单独修改个别部分
...
// b 复制父进程的虚拟地址池的位图
    ...
    if (vaddr_btmp == NULL)
        return -1;
    // 将 child_thread->userprog_vaddr.vaddr_bitmap.bits 指向自己的位图 vaddr_btmp
...
// b 为 switch_to 构建 struct thread_stack, 将其构建在邻近 intr_stack 之下的空间
